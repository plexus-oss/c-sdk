...
/*
 * Enforce the same pattern the gateway enforces at ingest time:
 *   ^[a-z0-9][a-z0-9._-]*$
 * First char must be lowercase-alphanumeric; remaining chars may additionally
 * be '.', '_', or '-'. Uppercase is rejected so the SDK fails fast with a
 * clear message instead of being rejected by the gateway's 400.
 */
